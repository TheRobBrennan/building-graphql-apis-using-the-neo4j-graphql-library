...
**This might be where we want to fetch data from another database, API, or system.**

Let’s consider a contrived example where we compute an estimated delivery date using a custom resolver function.

First, we add an `estimatedDelivery` field to the `Order` type, including the `@ignore` [directive](https://neo4j.com/docs/graphql-manual/current/type-definitions/access-control/#type-definitions-access-control-ignore) which indicates we plan to resolve this field manually and it will not be included in the generated database queries.
    ...
    "orders": [
      {
        ...
        "estimatedDelivery": "2021-09-05T00:03:48.871Z"
      },
      {
        "shipTo": {
          "address": "Nordenskiöldsgatan 24, 211 19 Malmö, Sweden"
        },
        "estimatedDelivery": "2021-09-09T00:03:48.871Z"
      }
    ]
  }
}
